...
import { getDomainsApi } from '../api/ssl'
import { ShieldCheck, Shield, XCircle, AlertCircle, ArrowRight } from 'lucide-react'
import type { CertStatus, CertRecord } from '../types/ssl'
...
function buildStats(certs: CertRecord[], loading: boolean): StatDef[] {
  ...
  return [
    {
      ...
      label: 'Total',
      value: v(total),
      desc:  total === 1 ? '1 certificate' : `${loading ? '—' : total} certificates`,
      color: 'var(--c-primary)',
  ...
  return (
    <main className="flex-1 p-5 lg:p-8 max-w-6xl w-full mx-auto space-y-5">
      ...
      <div className="pt-1">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--c-text-2)' }}>SSL certificate health overview</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        {/* Featured — total with recent list */}
        <div
        ...
        >
            ...
            <div>
              ...
              {!isLoading && domains.length > 0 && (
                <p className="text-xs mt-1" style={{ color: 'var(--c-text-3)' }}>
                  across {domains.length} {domains.length === 1 ? 'domain' : 'domains'}
                </p>
              )}
            </div>
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ background: 'var(--c-primary-soft)' }}>
          ...
          {!isLoading && allCerts.length > 0 && (
            <div className="space-y-2">
              {allCerts.slice(0, 3).map((c) => (
                <div
                  key={c._id}
                  className="flex items-center justify-between py-2 px-3 rounded-xl"
                  style={{ background: 'var(--c-surface)' }}
                >
                  <span className="font-mono text-xs" style={{ color: 'var(--c-text-1)' }}>{c.certName}</span>
                  <StatusDot status={c.status} />
                </div>
              ))}
              {allCerts.length > 3 && (
                <Link to="/certificates" className="flex items-center gap-1 text-xs px-3" style={{ color: 'var(--c-primary)' }}>
                  +{allCerts.length - 3} more <ArrowRight className="w-3 h-3" />
                </Link>
              )}
            </div>
          )}
      ...
      {!isLoading && recent.length > 0 && (
        <div className="rounded-2xl" style={{ background: 'var(--c-card)', border: '1px solid var(--c-border)' }}>
          <div className="flex items-center justify-between px-6 pt-5 pb-4" style={{ borderBottom: '1px solid var(--c-border)' }}>
            ...
            <Link to="/certificates" className="text-xs flex items-center gap-1" style={{ color: 'var(--c-primary)' }}>
              ...
              <tbody>
                {recent.map((cert) => (
                  <tr key={cert._id} className="hover">
                    <td>
                      <Link to={`/certs/${cert._id}`} className="font-mono text-sm hover:underline" style={{ color: 'var(--c-primary)' }}>
                        {cert.certName}
                      </Link>
                    </td>
                    <td>
...
function StatCard({ stat, isLoading }: { stat: StatDef; isLoading: boolean }) {
  ...
  return (
    <div className="rounded-2xl p-6 flex flex-col gap-3" style={{ background: 'var(--c-card)', border: '1px solid var(--c-border)' }}>
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--c-text-3)' }}>{label}</p>
